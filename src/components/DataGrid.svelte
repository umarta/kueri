<script lang="ts">
  ...
  /** UPDATE SQL preview generated by the parent — shown in the commit bar. */
  export let stagedSql: string = "";
  let sqlOpen = false;
...
{#if result}
  ...
  {#if pending.length}
    <div class="commit-bar" role="status">
      ...
      {#if stagedSql}
        <button class="btn-link" on:click={() => (sqlOpen = !sqlOpen)} title="Toggle SQL preview">
          {sqlOpen ? "Hide SQL" : "Preview SQL"}
        </button>
      {/if}
      <div class="spacer"></div>
      {#if undoable}
    ...
    {#if sqlOpen && stagedSql}
      <pre class="sql-preview">{stagedSql}</pre>
    {/if}
  {/if}
{:else}
  <div class="empty">
...
<style>
  ...
  .btn-link {
    font-size: 11.5px; color: var(--accent); padding: 2px var(--s-2);
    border-radius: var(--r-xs); background: none;
  }
  .btn-link:hover { background: var(--bg-elevated); }
  .sql-preview {
    margin: 0; padding: var(--s-3) var(--s-5);
    background: var(--bg-content); border-top: 1px solid var(--hairline);
    font-family: var(--font-mono); font-size: 11.5px; color: var(--ink-soft);
    white-space: pre-wrap; word-break: break-all; line-height: 1.5;
    max-height: 140px; overflow-y: auto; flex: none;
  }
